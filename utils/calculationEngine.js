  ...
  if (result.errors.length > 0) {
    ...
    if (currentIso.stop_sixth > minIso.stop_sixth) {
      ...
      if (bestIsoIndex !== newIsoIndex) {
        ...
        specificSuggestions.push(`💡 Réglage conseillé : ISO ${bestIso.display} (${cransDiff} cran${cransDiff > 1 ? 's' : ''} plus bas)`);
...
function calculateCorrection2(newApertureIndex, mainValues, settings) {
  ...
  if (result.errors.length > 0) {
    ...
    if (bestAperture && bestApertureIndex !== newApertureIndex) {
      specificSuggestions.push(`💡 Réglage conseillé : ${bestAperture.display} (${cransDiff} cran${cransDiff > 1 ? 's' : ''})`);
